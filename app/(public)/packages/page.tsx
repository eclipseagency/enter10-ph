...
const colorMap: Record<string, { border: string; glow: string; bg: string; accent: string }> = {
  'neon-blue': {
    ...
    glow: 'hover:shadow-[0_0_40px_rgba(0,212,255,0.18)]',
    bg: 'bg-neon-blue/10',
    accent: 'text-neon-blue',
  },
  'neon-gold': {
    border: 'border-t-neon-gold',
    glow: 'hover:shadow-[0_0_40px_rgba(255,184,0,0.18)]',
    bg: 'bg-neon-gold/10',
    accent: 'text-neon-gold',
  },
  'neon-magenta': {
    border: 'border-t-neon-magenta',
    glow: 'hover:shadow-[0_0_40px_rgba(255,45,120,0.18)]',
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10">
          {PACKAGES.map((pkg, i) => {
            ...
            return (
              <motion.div
                key={pkg.id}
                className={`glass rounded-2xl border-t-4 ${colors.border} flex flex-col overflow-hidden transition-all duration-300 ${colors.glow}`}
                custom={i}
                initial="hidden"
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm text-text-muted mb-6 pb-6 border-b border-border">
                  ...
                  <div className="mb-8 flex-1">
                    <p className="text-xs uppercase tracking-wider text-text-dim font-semibold mb-4">
                      {t('packages.includes')}
                    </p>
